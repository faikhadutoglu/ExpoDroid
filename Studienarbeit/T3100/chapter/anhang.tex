...
%		
%		\hyperref[cha:Interaktionsdesign]{3.5. Interaktionsdesign} & Simon Steiger \\
%		
%		\hyperref[cha:Display-Integration]{3.5.1. Display-Integration} & Simon Steiger \\
%		
...
%		
%		\hyperref[cha:Systemarchitektur]{3.6. Systemarchitektur} & Simon Steiger \\
%		
%		\hyperref[cha:Komponentenauswahl]{3.6.1. Komponentenauswahl} & Simon Steiger \\
%		
...
%	
%	\begin{tabular}{p{10cm} l}
%		\hyperref[cha:CAD-Modellierung]{4.1.1. CAD-Modellierung} & Simon Steiger \\
%		
%		\hyperref[cha:Montage und Nachbearbeitung]{4.1.2. Montage und Nachbearbeitung} & Simon Steiger \\
%		
%		\hyperref[cha:Motorsteuerung und Bewegungsalgorithmen]{4.2. Motorsteuerung und Bewegungsalgorithmen} & Faik Hadutoglu \\
%		
%		\hyperref[cha:Fernsteuerungssystem und Signalübertragung]{4.2.1. Fernsteuerungssystem und Signalübertragung} & Simon Steiger \\
%		
...
%		
%		\hyperref[cha:Integration]{4.4.4. Integration} & Simon Steiger \\
%		
%		\hyperref[cha:Testumgebung und Aufbau]{4.4.5. Testumgebung und Aufbau} & Simon Steiger \\
%		
%		\hyperref[cha:Testfälle und Akzeptanzkriterien]{4.4.6. Testfälle und Akzeptanzkriterien} & Simon Steiger \\
%		
%		\hyperref[cha:Beobachtungen und Probleme]{4.4.7. Beobachtungen und Probleme} & Simon Steiger \\
%		
%		\hyperref[cha:Geplante Netzwerktests (SSH/VNC)]{4.4.8. Geplante Netzwerktests (SSH/VNC)} & Simon Steiger \\
%		
%		\hyperref[cha:Fehleranalyse und Optimierung]{4.4.9. Fehleranalyse und Optimierung} & Simon Steiger \\
%		
%		\hyperref[cha:Ergebnisse und Diskussion]{5. Ergebnisse und Diskussion} & Faik Hadutoglu \\
%		
%		\hyperref[cha:Funktionsnachweis und Leistungsbewertung]{5.1. Funktionsnachweis und Leistungsbewertung} & Faik Hadutoglu \\
%		
%		\hyperref[cha:Antrieb und Fahrverhalten]{5.1.1. Antrieb und Fahrverhalten} & Simon Steiger \\
%		
%		\hyperref[cha:Interaktion und Aktorik]{5.1.2. Interaktion und Aktorik} & Simon Steiger \\
%		
%		\hyperref[cha:Systemintegration]{5.1.3. Systemintegration} & Simon Steiger \\
%		
%		\hyperref[cha:Anforderungsabgleich]{5.2. Anforderungsabgleich} & Faik Hadutoglu \\
%		
%		\hyperref[cha:Lessons Learned]{5.3. Lessons Learned} & Faik Hadutoglu \\
%		
%		\hyperref[cha:Kritische Reflexion]{5.4. Kritische Reflexion} & Simon Steiger \\
%		
%		\hyperref[cha:Mögliche Erweiterungen]{5.5. Mögliche Erweiterungen} & Faik Hadutoglu \\
%		
...
\end{lstlisting}
\begin{lstlisting}[language=Python, caption={MediaPipe Iris Tracking für Augenbewegung}, label={lst:mediapipe-eyes}]
	import cv2
	import numpy as np
	...
	LEFT_EYE_CENTER  = (int(CANVAS_W * 0.35), int(CANVAS_H * 0.50))
	RIGHT_EYE_CENTER = (int(CANVAS_W * 0.65), int(CANVAS_H * 0.50))
	EYE_RADIUS       = 100
	PUPIL_RADIUS     = 30
	PUPIL_MAX_OFFSET = int(EYE_RADIUS * 0.45)
	...
	return (int(sum(p[0] for p in points)/len(points)),
	int(sum(p[1] for p in points)/len(points)))
	
	def eye_width(corner_l, corner_r):
	return max(1.0, np.hypot(corner_r[0]-corner_l[0], corner_r[1]-corner_l[1]))
	
	def clamp_pupil(eye_center, target, max_offset):
	ex, ey = eye_center
	tx, ty = target
	dx, dy = tx - ex, ty - ey
	dist = np.hypot(dx, dy)
	if dist <= 1e-6: return eye_center
	scale = min(1.0, max_offset/dist)
	return (int(ex + dx*scale), int(ey + dy*scale))
	
	def smooth(prev, new, alpha=SMOOTHING_ALPHA):
	if prev is None: return new
	px, py = prev; nx, ny = new
	return (int(px + alpha*(nx-px)), int(py + alpha*(ny-py)))
	
	def draw_eyes(canvas, left_center, right_center, left_pupil, right_pupil):
	canvas[:] = BG_COLOR
	cv2.circle(canvas, left_center,  EYE_RADIUS, (255,255,255), -1)
	cv2.circle(canvas, left_center,  EYE_RADIUS, (0,0,0), 3)
	cv2.circle(canvas, right_center, EYE_RADIUS, (255,255,255), -1)
	cv2.circle(canvas, right_center, EYE_RADIUS, (0,0,0), 3)
	cv2.circle(canvas, left_pupil,  PUPIL_RADIUS, (0,0,0), -1)
	cv2.circle(canvas, right_pupil, PUPIL_RADIUS, (0,0,0), -1)
	cv2.putText(canvas, "Druecke 'q' zum Beenden",(20,30),
	...
	left_pupil_smoothed = None
	right_pupil_smoothed = None
	...
	target_left_canvas  = LEFT_EYE_CENTER
	target_right_canvas = RIGHT_EYE_CENTER
	
	if result.multi_face_landmarks:
	lm = result.multi_face_landmarks[0].landmark
	left_pts = [to_px(lm[i], w, h) for i in LEFT_EYE_POINTS]
	right_pts= [to_px(lm[i], w, h) for i in RIGHT_EYE_POINTS]
	left_center_px  = avg_points(left_pts)
	right_center_px = avg_points(right_pts)
	left_eye_w  = eye_width(left_pts[0], left_pts[1])
	right_eye_w = eye_width(right_pts[0], right_pts[1])
	left_iris_pts  = [to_px(lm[i], w, h) for i in LEFT_IRIS_IDXS]
	right_iris_pts = [to_px(lm[i], w, h) for i in RIGHT_IRIS_IDXS]
	left_iris_center  = avg_points(left_iris_pts)
	right_iris_center = avg_points(right_iris_pts)
	
	if None not in (left_center_px,left_iris_center,right_center_px,right_iris_center):
	dlx = left_iris_center[0]-left_center_px[0]
	dly = left_iris_center[1]-left_center_px[1]
	drx = right_iris_center[0]-right_center_px[0]
	dry = right_iris_center[1]-right_center_px[1]
	left_norm  = (dlx/left_eye_w,  dly/left_eye_w)
	right_norm = (drx/right_eye_w, dry/right_eye_w)
	target_left_canvas  = (int(LEFT_EYE_CENTER[0]+left_norm[0]*PUPIL_MAX_OFFSET),
	int(LEFT_EYE_CENTER[1]+left_norm[1]*PUPIL_MAX_OFFSET))
	target_right_canvas = (int(RIGHT_EYE_CENTER[0]+right_norm[0]*PUPIL_MAX_OFFSET),
	int(RIGHT_EYE_CENTER[1]+right_norm[1]*PUPIL_MAX_OFFSET))
	
	left_pupil_smoothed  = smooth(left_pupil_smoothed,  target_left_canvas)
	right_pupil_smoothed = smooth(right_pupil_smoothed, target_right_canvas)
	left_pupil  = clamp_pupil(LEFT_EYE_CENTER,  left_pupil_smoothed,  PUPIL_MAX_OFFSET)
	right_pupil = clamp_pupil(RIGHT_EYE_CENTER, right_pupil_smoothed, PUPIL_MAX_OFFSET)
	
	canvas = np.full((CANVAS_H,CANVAS_W,3), BG_COLOR, dtype=np.uint8)
	draw_eyes(canvas, LEFT_EYE_CENTER, RIGHT_EYE_CENTER, left_pupil, right_pupil)
	cv2.imshow("Eyes Tracking (MediaPipe Iris)", canvas)
